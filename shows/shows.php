...
<head>
  <meta charset="UTF-8" />
  <title>Shows | <?=htmlspecialchars($config['site_title'])?> Program</title>
...
<body class="bg-gray-100 text-gray-800">
  <main class="flex-1 w-full max-w-6xl px-4 py-10 mx-auto">
    ...
    <div class="flex justify-between items-center mb-6">
      <h2 class="text-2xl font-semibold text-[<?= htmlspecialchars($config['text_colour']) ?>]">Production List</h2>
      <a href="/shows/add/" class="bg-[<?= htmlspecialchars($config['button_colour']) ?>] hover:bg-[<?= htmlspecialchars($config['button_hover_colour']) ?>] text-white px-4 py-2 rounded transition">+ Add Show</a>
    </div>
