...
if (!$is_manager) die('<p class="text-center text-red-600 mt-10 font-semibold">Access Denied.</p>');

$id = isset($_GET['id']) ? (int)$_GET['id'] : 0;

// Load rehearsal
$stmt = $pdo->prepare("SELECT * FROM rehearsals WHERE id = ? AND show_id = ?");
$stmt->execute([$id, $show_id]);
$rehearsal = $stmt->fetch(PDO::FETCH_ASSOC);
if (!$rehearsal) die('Rehearsal not found.');
...
// Current attendees
$stmt = $pdo->prepare("SELECT user_id FROM rehearsal_attendees WHERE rehearsal_id = ?");
$stmt->execute([$id]);
$currentAttendees = array_map('intval', $stmt->fetchAll(PDO::FETCH_COLUMN));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($title === '' || $date === '' || $start_time === '' || $end_time === '') {
    ...
    $pdo->prepare("UPDATE rehearsals SET title = ?, date = ?, start_time = ?, end_time = ?, location = ?, notes = ? WHERE id = ? AND show_id = ?")
        ->execute([$title, $date, $start_time, $end_time, $location, $notes, $id, $show_id]);
    ...
    if ($selectAll) {
      ...
      foreach ($attendees as $uid) $ins->execute([$id, (int)$uid]);
    }
...
<head>
  <meta charset="UTF-8">
  <title>Edit Rehearsal | <?= htmlspecialchars($config['site_title']) ?></title>
  <script src="https://cdn.tailwindcss.com"></script>
</head>
<body class="bg-gray-100 text-gray-800">
  <main class="max-w-xl mx-auto mt-10 bg-white p-6 rounded shadow">
    <h1 class="text-2xl font-bold mb-4 text-[<?= $textColour ?>]">Edit Rehearsal</h1>
    <?php if ($error): ?><p class="text-red-600 mb-4"><?= htmlspecialchars($error) ?></p><?php endif; ?>

    <form method="POST" class="space-y-4">
      <div>
        <label class="block font-semibold mb-1">Title</label>
        <input type="text" name="title" value="<?= htmlspecialchars($rehearsal['title']) ?>" class="w-full border rounded p-2" required>
      </div>

      <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
        <div>
          <label class="block font-semibold mb-1">Date</label>
          <input type="date" name="date" value="<?= htmlspecialchars($rehearsal['date']) ?>" class="w-full border rounded p-2" required>
        </div>
        <div>
          <label class="block font-semibold mb-1">Start</label>
          <input type="time" name="start_time" value="<?= htmlspecialchars(substr($rehearsal['start_time'],0,5)) ?>" class="w-full border rounded p-2" required>
        </div>
        <div>
          <label class="block font-semibold mb-1">End</label>
          <input type="time" name="end_time" value="<?= htmlspecialchars(substr($rehearsal['end_time'],0,5)) ?>" class="w-full border rounded p-2" required>
        </div>
      </div>

      <div>
        <label class="block font-semibold mb-1">Location</label>
        <input type="text" name="location" value="<?= htmlspecialchars($rehearsal['location']) ?>" class="w-full border rounded p-2">
      </div>

      <div>
        <label class="block font-semibold mb-1">Notes</label>
        <textarea name="notes" class="w-full border rounded p-2"><?= htmlspecialchars($rehearsal['notes']) ?></textarea>
      </div>
      ...
      <div>
        ...
        <select name="attendees[]" id="attendees" class="w-full border rounded p-2" multiple size="10">
          <?php foreach ($people as $p): ?>
            <option value="<?= $p['id'] ?>" <?= in_array((int)$p['id'], $currentAttendees) ? 'selected' : '' ?>>
