...
<body class="bg-gray-100 text-gray-800 flex flex-col min-h-screen">
  ...
  <main class="flex-1 w-full max-w-6xl px-4 py-10 mx-auto">
    <div class="bg-white rounded-xl shadow p-6 border border-gray-200">
      <h1 class="text-2xl font-bold text-[<?= htmlspecialchars($config['text_colour']) ?>] mb-4">🎭 Link Teachers</h1>
      ...
      <form method="POST" class="space-y-4">
        ...
        <button type="submit" class="bg-[<?= htmlspecialchars($config['button_colour']) ?>] hover:bg-[<?= htmlspecialchars($config['button_hover_colour']) ?>] text-white px-4 py-2 rounded shadow">
          Link Teacher
        </button>
      </form>
    </div>
