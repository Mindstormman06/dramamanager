...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $stmt = $pdo->prepare("UPDATE characters SET stage_name = ?, real_name = ?, show_id = ? WHERE id = ?");
        $stmt->execute([$stage_name, $real_name ?: null, $show_id, $id]);
        header("Location: characters.php");
        exit;
    }
}
...
<header class="bg-purple-800 text-white py-6 mb-8 shadow-md">
  <div class="max-w-4xl mx-auto px-4">
    <h1 class="text-3xl font-bold">✏️ Edit Character</h1>
